...
install_centos_ssr(){
	...
	ldconfig
	git clone -b manyuser https://github.com/glzjin/shadowsocks.git "/shadowsocks"
	cd /shadowsocks
...
install_node(){
	...
	sed -i "s#'zhaoj.in'#'jd.hk'#" /shadowsocks/userapiconfig.py
	Userdomain=${Userdomain:-"http://127.0.0.1"}
	sed -i "s#https://zhaoj.in#${Userdomain}#" /shadowsocks/userapiconfig.py
	Usermukey=${Usermukey:-"mupass"}
	sed -i "s#glzjin#${Usermukey}#" /shadowsocks/userapiconfig.py
	UserNODE_ID=${UserNODE_ID:-"3"}
	sed -i '2d' /shadowsocks/userapiconfig.py
	sed -i "2a\NODE_ID = ${UserNODE_ID}" /shadowsocks/userapiconfig.py
	# 启用supervisord
	supervisorctl shutdown
	#某些机器没有echo_supervisord_conf 
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/ZJY2003/thessr-ssr-node/master/supervisord.conf
	supervisord
	#iptables
	iptables -F
	iptables -X  
	...
	firewall-cmd --add-port=10000-65535/tcp --permanent
	firewall-cmd --add-port=10000-65535/udp --permanent
	firewall-cmd --save-all

	echo 'iptables-restore /etc/sysconfig/iptables' >> /etc/rc.local
	wget -N -P  /usr/lib/systemd/system/ --no-check-certificate  https://raw.githubusercontent.com/ZJY2003/thessr-ssr-node/master/supervisord.service
...
echo "# Github: https://github.com/mmmwhy/ss-panel-and-ss-py-mu   #"
echo "# Author: 91vps & Tony Zou                                  #"
echo "# Press enter and the installation will start               #"
echo "#############################################################"
echo
num=$1
if [ "${num}" == "1" ]; then
